    ...
    <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
      ...
      <div className="max-w-7xl mx-auto px-8 sm:px-12 lg:px-20">
        ...
        <div className="text-center mb-16">
          <p className="text-[#C9A84C] text-[10px] tracking-[.55em] mb-5">BROWSE ALL</p>
          <h1 className="font-['Cormorant_Garamond',serif] text-5xl sm:text-6xl lg:text-7xl text-white mb-7">
        ...
        <div className="flex gap-3 mb-8">
          <div className="relative flex-1">
            <Search size={13} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none" />
            <input type="text" value={search} onChange={e=>setSearch(e.target.value)}
              placeholder="Search jewellery…"
              className="w-full bg-transparent border border-white/10 py-3 pl-10 pr-10
                         text-xs text-gray-200 placeholder-gray-600 tracking-wider
                         outline-none focus:border-[#C9A84C]/50 transition-colors" />
            {search && (
          ...
          <div className="relative flex-shrink-0">
            <button onClick={() => setShowSort(v=>!v)}
              className="flex items-center gap-2 border border-white/10 px-5 py-3
                         text-xs text-gray-400 tracking-widest hover:border-white/30 hover:text-gray-200 transition-colors">
            ...
            <AnimatePresence>
              {showSort && (
                <motion.div initial={{ opacity:0,y:6 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:6 }}
                  className="absolute right-0 top-full mt-2 bg-[#111] border border-white/10 z-20 min-w-[190px]">
                  {SORTS.map(([v,l]) => (
                    <button key={v} onClick={() => { setSortBy(v); setShowSort(false); }}
                      className={`w-full text-left px-5 py-4 text-xs tracking-wider border-b border-white/5 last:border-0 transition-colors ${
                        sortBy===v ? 'text-[#C9A84C] bg-[#C9A84C]/5' : 'text-gray-400 hover:text-white hover:bg-white/3'
                      }`}>{l}
                    </button>
                  ))}
                </motion.div>
        ...
        {/* count */}
        {!loading && (
          <p className="text-gray-600 text-[10px] tracking-widest mb-10">
            {filtered.length} {filtered.length===1?'PIECE':'PIECES'} FOUND
            {cat!=='All' && <span className="ml-2 text-[#C9A84C]/50">IN {cat.toUpperCase()}</span>}
          </p>
        )}
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {Array.from({length:8}).map((_,i) => (
              <div key={i} className="aspect-[3/4] bg-white/4 animate-pulse" />
            ))}
          </div>
        ) : filtered.length===0 ? (
          <div className="text-center py-32">
            <p className="text-5xl mb-6">✦</p>
            <h3 className="font-['Cormorant_Garamond',serif] text-3xl text-white italic mb-4">Nothing Found</h3>
            <p className="text-gray-400 text-sm mb-8">
              {search ? `No jewellery matching "${search}"` : 'No pieces in this category yet.'}
            </p>
            {(search||cat!=='All') && (
              <button onClick={() => { setSearch(''); setCat('All'); }}
                className="text-[10px] tracking-widest text-[#C9A84C] border border-[#C9A84C]/30 px-6 py-3 hover:bg-[#C9A84C]/5 transition-colors">
                CLEAR FILTERS
              </button>
            )}
          </div>
